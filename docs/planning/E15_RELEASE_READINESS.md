...
- A single artifact-set orchestrator now preflights clean/pushed source and external signing prerequisites, securely reuses one password input across the historical baseline and both candidates, runs the artifact-only release gate, and writes a secret-free hash manifest. The real artifact set still awaits a run with the signing passwords available.
