...
- A single artifact-set orchestrator now preflights clean/pushed source and external signing prerequisites, securely reuses one password input across the historical baseline and both candidates, runs the artifact-only release gate, and writes a secret-free hash manifest. Its self-check rejects a wrong HEAD/baseline, missing or changed artifact, escaped path, changed preflight log, or nested artifact-only hash mismatch. A failed historical Unity build preserves only ignored `*.log` diagnostics before removing its temporary worktree. The real artifact set still awaits a successful run.
