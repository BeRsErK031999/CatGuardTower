...
- The signed candidate wrapper writes secret-free APK/AAB provenance bound to a clean Git HEAD, Unity version, artifact hash, and restored project settings. Its Windows Gradle child uses an owned short temporary path to avoid the JDK AF_UNIX loopback failure and redacts its retained log. The baseline wrapper separately binds the universal `0.1.0` APK to historical commit `28f7e88`, the current clean orchestration HEAD, clean/restored historical sources, and the intermediate AAB/APKS transformation; on historical Unity failure it preserves redacted ignored log-only diagnostics before worktree cleanup. The signed-artifact runner verifies all three sidecars plus package/version, SDK levels, ABI, certificate continuity, AAB validation, manifest permissions, clean install, offline launch, baseline upgrade, save migration, landscape, crashes, and physical performance evidence.
- The artifact-set orchestrator fails before prompting or building unless the repository is clean and pushed and an external post-incident record binds the selected rotated JKS plus a schema-v1 current-user DPAPI bundle with independent store/key `SecureString` values. Both retired file fingerprints, the legacy one-password `PSCredential`, and manual/environment password sources are rejected. Registration first proves that the bundle opens the selected alias through Unity's bundled `keytool` and that its actual certificate digest is the prepared upload key; the record also binds the verifier hash and method. It then builds the baseline plus candidate APK/AAB from one immutable HEAD and immediately runs the artifact-only release gate. Its ignored manifest binds the signing-rotation proof, resulting six artifact/provenance files, and preflight evidence by SHA-256, then validates those bytes and the nested artifact-only source binding before reporting success.
