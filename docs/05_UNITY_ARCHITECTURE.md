...
- `AchievementService` and its pure `AchievementStateMachine` own the E11 long-term-goal lifecycle: reliable retroactive synchronization, battle/daily/garden events, hidden presentation state, completion dates, bounded event deduplication, rewarded-revive rollback, and one-time Fish/XP claims. `AchievementCatalogConfig` is the source of truth for all 12 ids, rules, targets, tiers, categories, localization keys, and reward bundles.
- `CampaignMapMetadata` and `CampaignChallengeConfig` own the E12 replay contract. `ProgressionService` separates normal map completion from challenge completion, while `PrototypeLevelController` applies generic modifiers without content-id checks.
- `BossEncounterConfig` and `BossRuntimeController` own E13 phase thresholds, telegraphs, support waves, damage/status response, environment interaction, and boss cleanup. `AdvancedMapRuleConfig` and `AdvancedMapRuleController` own predictable route gates, temporary placement blocking, fog range changes, accessibility cues, and shared battle lifecycle cleanup.
- `BattlefieldPresentationPalette` is the source of truth for biome runtime colors; level/battlefield ids never select presentation behavior in the controller.
