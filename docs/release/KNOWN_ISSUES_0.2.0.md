...
- The current Emulator 37.1.11 SwiftShader reproduction measured the E14 worst case below budget; the host-GPU diagnostic terminated before a comparable result. Runners now label both emulator modes as diagnostic and prevent either from satisfying release acceptance. An exact-APK, physical-hardware `level_12` heavy-wave capture is still missing, so performance acceptance remains blocked.
