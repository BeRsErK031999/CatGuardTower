...
- A single artifact-set orchestrator now preflights clean/pushed source and external signing prerequisites, securely reuses one password input across the historical baseline and both candidates, runs the artifact-only release gate, and writes a secret-free hash manifest. Its self-check rejects a wrong HEAD/baseline, missing or changed artifact, escaped path, changed preflight log, or nested artifact-only hash mismatch. Windows Gradle children use an owned short temporary path to avoid the JDK AF_UNIX loopback failure, then restore `TEMP`/`TMP`; failed Unity builds redact secret-bearing environment fields and known signing values from ignored logs. The real artifact set still awaits a successful run.
...
- A fail-fast E15 block-gate orchestrator now checks external P0, owner, clean-worktree, and upstream-sync preconditions before any Unity or Android mutation. Its full mode requires explicit emulator/physical targets, store-package reset confirmation, and the prepared artifact-set manifest. Before mutation it binds the exact selected artifact paths and bytes to that manifest; after the technical checks it repeats the complete contract so the manifest, artifacts, provenance, and artifact-only evidence must all remain unchanged. The orchestrator runs the cold validators and technical gates in order, then writes one source/artifact-bound manifest before final release-document approval. The final manifest is accepted only when every required precondition and step occurs exactly once, all results pass, artifact/provenance plus artifact-set hashes match, and every evidence log remains inside the run directory with matching SHA-256.
...
- A candidate upload key now exists outside Git under `%USERPROFILE%\.catguard\release-signing`; its store/key passwords must be rotated and the DPAPI convenience credential refreshed before the next signed build. Independent backup and owner approval also remain open, so it does not yet close `STORE-ACCOUNT-001`.
...
- `STORE-ACCOUNT-001`: partially prepared. Upload-key password rotation plus DPAPI refresh, final owner identity/contact, public privacy URL, audience/content rating, Play Console actions, screenshot/listing approval, and independent upload-key backup/approval are unavailable.
